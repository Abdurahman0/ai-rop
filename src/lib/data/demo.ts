...
export const demoCalls: Call[] = [
  { id: 1284, client_phone: "+998 90 214 88 12", operator: "Bobur", direction: "outbound", duration: 426, stage: "completed", started_at: new Date(Date.now() - 1000 * 60 * 35).toISOString(), provider: "telephony" },
  { id: 1281, client_phone: "+998 93 771 10 44", operator: "Azizbek", direction: "inbound", duration: 612, stage: "completed", started_at: new Date(Date.now() - 1000 * 60 * 90).toISOString(), provider: "telephony" },
  { id: 1277, client_phone: "+998 97 442 31 09", operator: "Dilshod", direction: "outbound", duration: 288, stage: "completed", started_at: new Date(Date.now() - 1000 * 60 * 180).toISOString(), provider: "telephony" },
  { id: 1272, client_phone: "+998 91 333 21 17", operator: "Sardor", direction: "inbound", duration: 742, stage: "completed", started_at: new Date(Date.now() - 1000 * 60 * 260).toISOString(), provider: "telephony" },
  { id: 1268, client_phone: "+998 99 110 41 20", operator: "Malika", direction: "outbound", duration: 0, stage: "analyzing", started_at: new Date(Date.now() - 1000 * 60 * 20).toISOString(), provider: "telephony" },
];

export const demoAnalyses: Analysis[] = [
  { id: 1, call: 1284, summary: "Price objection was not handled effectively", overall_score: 61, evaluation: "The seller acknowledged the concern but did not reframe value or confirm next steps.", extracted_fields: { budget: "Price-sensitive", intent: "Medium", objection: "Monthly cost" }, lead_created: false, skip_reason: "no_lead_intent", model_name: "gpt-sales-review", created_at: new Date(Date.now() - 1000 * 60 * 30).toISOString() },
  ...
  { id: 3, call: 1277, summary: "Customer showed buying intent but no follow-up action was identified", overall_score: 72, evaluation: "Intent signals were present. The seller should have scheduled a concrete next touch.", extracted_fields: { intent: "High", next_step: "Undefined" }, lead_created: false, skip_reason: "insufficient_data: mahsulot", model_name: "gpt-sales-review", created_at: new Date(Date.now() - 1000 * 60 * 170).toISOString() },
...
export const demoClients: Client[] = [
  { id: 1, name: "Akmal Rakhimov", phone: "+998 90 214 88 12", created_via: "call_ai", created_at: new Date(Date.now() - 86400000).toISOString(), updated_at: new Date().toISOString(), custom_data: { company: "UzTrade" } },
...
export const demoLeads: Lead[] = [
  { id: 1, title: "ERP automation consultation", client: demoClients[0], status: demoStatuses[1], source_call: demoCalls[1], assigned_to: "Azizbek", created_via: "call_ai", created_at: new Date(Date.now() - 9000000).toISOString(), updated_at: new Date().toISOString(), custom_data: { expected_budget: "$3k" } },
  { id: 2, title: "Call review follow-up", client: demoClients[1], status: demoStatuses[0], source_call: demoCalls[3], assigned_to: "Sardor", created_via: "call_ai", created_at: new Date(Date.now() - 12000000).toISOString(), updated_at: new Date().toISOString(), custom_data: { priority: "High" } },
];
